...
{
public:
    Train(Size size=9, uint32_t state_c=2, uint32_t n_thread=6, double lr=4e-3, double c_lr=1, double temp=1, uint32_t n_simulate=400,
...
void Train::run(const char *model_path_local, const char *best_path_local)
{
    ...
    for (i = 0; i < this->n_game; i++)
    {
        ...
        mcts.self_play(&this->nogo, states_local, probs_local, values_local, this->temp, 10, true, false);
